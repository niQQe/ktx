...
- 🕹️ **Antilag 2** — the official QW-Group MVDSV-native antilag (`sv_antilag 2`, engine-side rewind). The rewind logic itself does not run `antilag.c`'s code — **but see the crash/glitch section below: this does NOT mean it works safely on any stock MVDSV out of the box.**
...
## 🐛 Known bug: projectile visual glitch (jumps/duplicates), separate from the crash above, present in **all three antilag modes**

**Observed while testing** (with a working `SetSendNeeded`, i.e. against dusty-qw/mvdsv, so no crash): with `sv_antilag 2` and a connected client, thrown grenades visually **jump/teleport or appear duplicated** in flight. Code inspection confirms this affects **every antilag mode, including antilag fully off (`sv_antilag 0`), and every native projectile type** (rocket, grenade, nail, super-nail) — not just grenades, not just Antilag 2.

**Root cause, confirmed by code inspection — inherited from dusty-qw/ktx `master` unmodified:**

- The same unconditional `ScheduleProjectileSendIfLive()` calls described in the crash section above register every projectile as a CSQC-networked entity **regardless of `sv_antilag`'s value**, and `UpdateProjectileSendNeeded()` (every server frame, `world.c`'s `StartFrame`) keeps re-sending origin/velocity/time via `MSG_CSQC` for as long as it's alive — again with no `sv_antilag` check.
- Only the **rewind/lag-compensation** side (`antilag_lagmove_all_proj_bounce()` and friends in `src/antilag.c`) is correctly gated on `if (cvar("sv_antilag") != 1) return;`. **The CSQC networking switch and the rewind switch are independent, and only the rewind one is wired to the `sv_antilag` cvar.**
- Net effect (once the crash above is out of the way, e.g. on dusty-qw/mvdsv): **in Antilag 0, 1, or 2**, the server always sends CSQC entity updates for every projectile. A client that renders CSQC entities ends up drawing the projectile from **two sources at once** — the normal QW baseline/delta update and the CSQC mirror — which disagree on interpolation, causing the jump/duplicate artifact. **This means Antilag 0 in this port is not visually identical to stock QW-Group KTX for any CSQC-aware client**, even though the rewind/gameplay logic itself is correctly disabled.
- Confirmed **not** a QW-Group/ktx behavior: `src/antilag.c` doesn't exist at all in [QW-Group/ktx](https://github.com/QW-Group/ktx) upstream.

**Not fixed in this PR** (per the "unmodified port, document don't fix" scope decision). Smallest safe fix would be gating `ScheduleProjectileSendIfLive()`/`UpdateProjectileSendNeeded()` behind `sv_antilag == 1` (or a per-client CSQC-capability check) so modes 0 and 2 match stock QW-Group KTX for every client. Happy to make that change if desired.
...
- **Runtime, with a real connected client**: ran this KTX build against a freshly-built **dusty-qw/mvdsv `master`** (chosen over `antilag-new`, which turned out to be stale relative to `master`), map `aerowalk`, both `sv_antilag 1` and `sv_antilag 2`. Server stayed up, a real player connected and played (no crash observed in this session, since dusty-qw/mvdsv implements `SetSendNeeded` for real). **Found the projectile CSQC visual bug documented above** while doing this.
