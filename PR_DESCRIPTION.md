...
## 🐛 Known bug: projectile visual glitch (jumps/duplicates) in **all three modes** — Antilag 0, 1, and 2

**Observed while testing**: with `sv_antilag 2` (QW-Group native antilag, KTX rewind code supposedly inactive) and a connected client, thrown grenades visually **jump/teleport or appear duplicated** in flight. Code inspection confirms this is **not limited to grenades or to Antilag 2** — it affects **every antilag mode, including antilag fully off (`sv_antilag 0`), and every native projectile type** (rocket, grenade, nail, super-nail).
...
- All four projectile-spawning paths in `src/weapons.c` — `W_FireRocket()`, `W_FireGrenade()`, and `launch_spike()` (used by both `W_FireSpikes()`/nailgun and `W_FireSuperSpikes()`/super nailgun) — unconditionally call `ScheduleProjectileSendIfLive(newmis)`, **regardless of `sv_antilag`'s value (0, 1, or 2)**. This registers the projectile as a CSQC-networked entity (`ExtFieldSetSendEntity(projectile, SendEntity_Projectile)` + `SetSendNeeded(projectile, PROJECTILE_INITIAL, 0)`).
- `UpdateProjectileSendNeeded()` (called every server frame from `world.c`'s `StartFrame`, again with **no `sv_antilag` check**) keeps re-sending grenade origin/velocity/time via `MSG_CSQC` for as long as it's alive.
- Only the **rewind/lag-compensation** side (`antilag_lagmove_all_proj_bounce()` and friends in `src/antilag.c`) is correctly gated on `if (cvar("sv_antilag") != 1) return;` — confirmed by direct inspection, matches `dusty-qw/ktx` master line-for-line. **The CSQC networking side and the rewind side are two independent switches, and only the rewind one is actually wired to the `sv_antilag` cvar.**
- Net effect: **in Antilag 0 (off), Antilag 2, or Antilag 1**, the server always sends CSQC entity updates for every projectile via `MSG_CSQC`/`SendEntity_Projectile`. A client that renders CSQC entities (e.g. an EZCSQC-aware ezquake build) ends up drawing the projectile from **two sources at once**: the normal QW baseline/delta entity update, and the CSQC projectile mirror — which don't agree on interpolation, causing the jump/duplicate artifact. **This means Antilag 0 in this port is not visually identical to stock QW-Group KTX for any client that understands CSQC**, even though the gameplay/rewind logic itself is correctly disabled.
- Compounding factor in the specific session where this was first observed: the `csprogs.dat` loaded on the test server was, at that point, a stale prebuilt binary (~2 days older than the `qcsrc/*.qc` sources in this port) from an unrelated older project folder, so the client-side CSQC code and the server's wire format may not have agreed — this alone could have produced additional visual desync independent of the above. **This was an artifact of that specific local test setup, not something this PR ships** (this PR does not include a prebuilt `csprogs.dat` — see the "Antilag 1 — CSQC/EZCSQC layer" section above). We since rebuilt `csprogs.dat` directly from this PR's own `qcsrc/*.qc` (via `fteqcc`) and re-tested; **the underlying always-on-CSQC-projectile behavior described above is a code-level fact independent of which `csprogs.dat` build is used** — it's not something a matching build makes go away.
- **Confirmed this is not a QW-Group/ktx behavior**: `src/antilag.c` (where the rewind logic lives) does not exist at all in [QW-Group/ktx](https://github.com/QW-Group/ktx) upstream, so this always-on projectile-CSQC behavior is dusty-qw-specific, present in `master` before this port and carried over unmodified per the "no fixes during port" scope decision for this PR.

**This was not fixed in this PR** (per the earlier decision to port dusty-qw/ktx unmodified and document risks instead of patching them). If this needs a fix, the smallest safe change would be gating `ScheduleProjectileSendIfLive()`/`UpdateProjectileSendNeeded()` behind `sv_antilag == 1` (or behind a per-client CSQC-capability check) so Antilag 0 and Antilag 2 behave identically to stock QW-Group KTX for every client. Happy to make that change if desired — just say so, since it would be a deviation from the "unmodified port" approach used everywhere else in this PR.
...
## 🔍 Coverage audit — everything Antilag 1 and Antilag 2 need, present and verified

Re-audited before finalizing this PR:

- **File coverage**: diffed the complete file list from `dusty-qw/ktx`'s `non-antilag..master` range (excluding sprays) against every file touched by this PR. **Only `qcsrc/fteqcc64` is missing** — a prebuilt QuakeC compiler binary, deliberately excluded (see the "Antilag 1 — CSQC/EZCSQC layer" section above); everything else is ported.
- **Antilag 1 (KTX side) build**: clean from-scratch build of `qwprogs.so` (0 errors) and a clean from-scratch `fteqcc` compile of `qcsrc/*.qc` into `csprogs.dat` (0 warnings), both from this PR's current commit.
- **Antilag 2 (MVDSV side) parity**: diffed `dusty-qw/mvdsv`'s `master` directly against `QW-Group/mvdsv`'s `master` for every file that implements native antilag (`sv_phys.c`, `pr_cmds.c`, `sv_user.c`) — the `sv_antilag`/`sv_antilag_no_pred`/`sv_antilag_projectiles` cvars and the trace/physics logic that reads them are **byte-for-byte identical**; dusty-qw only adds new code behind `#ifdef`s (EZCSQC, sprays) alongside it, it doesn't touch the existing antilag-2 code path. (One unrelated non-antilag change was found and is called out separately below: `safestrafe`.)
- **Runtime**: this branch's `qwprogs.so`, freshly built `dusty-qw/mvdsv` `master`, and a `csprogs.dat` rebuilt directly from this PR's own `qcsrc/*.qc` were all run together with a real connected client, testing `sv_antilag 1` and `sv_antilag 2` (server stayed up, no crash in this session — see Validation section for exact scope/limits).
- **Sync check**: confirmed the branch pushed to this PR (`port/antilag-1-and-2`) matches the local working copy and the private staging repo (`tibazera/antilagezqc-qwleague`) exactly — no divergence between what was tested and what's proposed here.
