...
## ℹ️ Projectile CSQC networking is not gated by antilag mode (code fact) — visual glitch only seen in Antilag 0/2, high ping, likely client-side
...
**Observed symptom, narrowed down across multiple test sessions**: a visual glitch (grenades/rockets appearing to jump/duplicate) shows up specifically under **`sv_antilag 0` and `sv_antilag 2`**, correlates with **high ping**, and does **not** occur under `sv_antilag 1`. A clean local (low-latency) retest with `csprogs.dat` rebuilt directly from this PR's own `qcsrc/*.qc` did not reproduce it, consistent with the high-ping correlation.

This pattern lines up with the code-level fact above and gives a specific, plausible mechanism: in Antilag 1, the client is expected to trust the CSQC projectile mirror as authoritative (that's the whole point of EZCSQC). In Antilag 0/2, the client is *not* told to defer to CSQC for these entities — it still gets the normal QW baseline/delta entity update **at the same time** as the CSQC mirror update, and the two can disagree on interpolated position, with the disagreement more visible the more prediction/extrapolation error accumulates (i.e. at higher ping). This would point at **client-side (unezquake) interpolation/reconciliation logic** as the likely place the glitch actually manifests, rather than a server-side KTX/mvdsv defect — though this hasn't been independently confirmed by us (we don't have a suitable high-latency test setup here), and we haven't ruled out a subtler server-side contribution either.

**Net assessment**: downgrading this from "confirmed KTX bug" to "known code-level quirk (always-on CSQC networking, not gated by `sv_antilag`) whose visible symptom is Antilag-0/2-only, high-ping-dependent, and most likely a client-side (unezquake) interpolation issue rather than a server-side defect.**

If you still want this gated server-side regardless (so Antilag 0/2 send zero CSQC projectile traffic, matching stock QW-Group KTX byte-for-byte on the wire, independent of what any given client does with it), the smallest safe fix would be `ScheduleProjectileSendIfLive()`/`UpdateProjectileSendNeeded()` behind `sv_antilag == 1`. Happy to make that change if desired — just say so, since it's a deviation from the "unmodified port" approach used elsewhere in this PR.
